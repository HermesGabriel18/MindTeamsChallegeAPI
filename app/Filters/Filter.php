...
abstract class Filter
{
     ...
     */
    public function getOrPaginate(Builder $builder, ?int $perPage = self::PAGE_SIZE): Collection|LengthAwarePaginator|\Illuminate\Support\Collection
    {
     ...
     * @return Builder
     */
    protected function ids($ids): Builder
    {
     ...
     * @return Builder
     */
    protected function name($name): Builder
    {
        if ($this->builder->getModel() instanceof HasNameInterface) {
     ...
     * @return Builder
     */
    protected function disabled($disabled): Builder
    {
        if ($this->builder->getModel() instanceof HasDisabledInterface) {
            return $this->filterNullable('disabled', $disabled);
        }
     ...
     * @return Builder
     */
    protected function createdBy($ids): Builder
    {
        if (!$ids) {
            return $this->builder;
        }
        ...
        if ($this->builder->getModel() instanceof HasCreatedByInterface) {
    ...
    private function applyOrder(?string $sortString = null)
    {
        if ($sortString && str_contains($sortString, '|')) {
            ...
            $this->builder->orderBy('id', 'desc'); // default ordering
        }
    }
